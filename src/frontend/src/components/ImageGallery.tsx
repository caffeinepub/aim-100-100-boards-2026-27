...
function useImageBank(bankKey: string) {
  const [slots, setSlots] = useState<ImageSlot[]>(() => {
    ...
    return Array(BANK_SIZE)
      .fill(null)
      .map(() => ({ dataUrl: null, name: "" }));
  });
...
function ImageBank({ bankKey }: BankProps) {
  ...
  const handleSlotClick = (globalIndex: number) => {
    if (slots[globalIndex].dataUrl) {
      setSelectedImage({
        url: slots[globalIndex].dataUrl!,
        name: slots[globalIndex].name,
      });
    } else {
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-1">
          {Array.from({ length: Math.min(totalPages, 10) }, (_, i) => i).map(
            (pageNum) => (
              <Button
                key={pageNum}
                ...
                style={
                  page === pageNum ? { background: "oklch(0.65 0.17 55)" } : {}
                }
              >
                {pageNum + 1}
              </Button>
        ...
        {pageSlots.map((slot, localIndex) => {
          ...
          return (
            <button
              ...
              className="aspect-square rounded-lg border border-border cursor-pointer overflow-hidden relative group transition-all duration-200 hover:border-primary hover:shadow-md p-0"
              style={{
      ...
      {selectedImage && (
        <button
          type="button"
          className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 border-0 w-full"
