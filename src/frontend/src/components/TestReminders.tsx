...
export default function TestReminders() {
  const { reminders, addReminder, deleteReminder, getTestsDueSoon } =
    useTestReminders();
        ...
        <Alert className="border-orange-500/50 bg-orange-500/10">
          ...
          <AlertDescription className="text-orange-700 dark:text-orange-400">
            ...
            {dueSoon
              .map(
                (t) =>
                  `${t.subject} (${getDaysUntil(t.testDate) <= 0 ? "TODAY" : `${getDaysUntil(t.testDate)} day(s)`})`,
              )
              .join(", ")}
          </AlertDescription>
        </Alert>
        ...
        {reminders.length === 0 && (
          <div className="text-center py-12 text-muted-foreground">
            ...
            <p className="text-sm mt-1">
              Add your monthly test dates to stay on track!
            </p>
          </div>
        )}
        {reminders.map((reminder) => (
          <Card key={reminder.id} className="premium-card">
            <CardContent className="p-4 flex items-center gap-3">
              <div
                className={`w-3 h-3 rounded-full flex-shrink-0 ${getUrgencyColor(reminder.testDate)}`}
              />
        ...
        <Card className="premium-card bg-muted/30">
          <CardContent className="p-4">
            <p className="text-sm text-muted-foreground text-center">
              💡 <strong>Tip:</strong> Schedule monthly tests for each subject
              to stay consistent. Tests due within 3 days will show an alert on
              your dashboard!
            </p>
          </CardContent>
        </Card>
