...
export default function PomodoroTimer() {
  ...
  const handleSaveSettings = () => {
    const f = Math.max(1, Number.parseInt(focusMin) || 25);
    const b = Math.max(1, Number.parseInt(breakMin) || 5);
    const lb = Math.max(1, Number.parseInt(longBreakMin) || 15);
    ...
    <div className="max-w-lg mx-auto space-y-4 animate-fade-in">
          ...
          <div className="relative w-56 h-56">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              ...
              <p className="text-muted-foreground text-sm mt-1">
                Session{" "}
                {Math.floor(sessionCount / 1) + (mode === "focus" ? 1 : 0)}
              </p>
            </div>
          </div>
      ...
      <Card className="premium-card">
        <CardContent className="p-4">
          <p className="text-sm text-muted-foreground">
            💡 <strong>Pomodoro Tip:</strong> After every 4 focus sessions, take
            a 15-minute long break. Stay hydrated and avoid your phone during
            focus time!
          </p>
        </CardContent>
      </Card>
