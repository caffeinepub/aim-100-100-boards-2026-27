...
import { Flag, Pause, Play, Plus, RotateCcw, Trash2 } from "lucide-react";
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        {alarms.map((alarm) => (
          <Card
            key={alarm.id}
            className={`premium-card ${alarm.triggered ? "border-yellow-500 animate-pulse" : ""}`}
          >
            <CardContent className="p-4 flex items-center gap-3">
              <div className="flex-1">
                ...
                {alarm.triggered && (
                  <Badge className="text-xs mt-1 bg-yellow-500">
                    🔔 Ringing!
                  </Badge>
                )}
              </div>
...
function StopwatchTab() {
  const { isRunning, elapsed, laps, start, pause, reset, lap, formatTime } =
    useStopwatch();
