...
function formatMessage(text: string) {
  ...
  return lines.map((line, i) => {
    const stableKey = `line-${i}-${line.slice(0, 12)}`;
    if (line.startsWith("**") && line.endsWith("**")) {
      return (
        <p
          key={stableKey}
          className="font-bold text-foreground mt-2 first:mt-0"
        >
          {line.slice(2, -2)}
        </p>
      );
    ...
    if (line.startsWith("- ")) {
      return (
        <li key={stableKey} className="ml-4 list-disc">
    ...
    return <p key={stableKey}>{formatInline(line)}</p>;
  });
}
...
function formatInline(text: string) {
  ...
  return parts.map((part, i) => {
    const stableKey = `part-${i}-${part.slice(0, 12)}`;
    if (part.startsWith("**") && part.endsWith("**")) {
      return <strong key={stableKey}>{part.slice(2, -2)}</strong>;
    }
    return part;
  });
}
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-4 animate-fade-in">
              ...
              >
                ...
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
                    msg.sender === "ai" ? "bg-primary/20" : "bg-muted"
                  }`}
                >
                  {msg.sender === "ai" ? (
                    <Bot
                ...
                <div
                  ...
                  style={
                    msg.sender === "user"
                      ? {
                          background:
                            "linear-gradient(135deg, oklch(0.55 0.17 45), oklch(0.65 0.17 55))",
                        }
                      : {}
                  }
                >
                  ...
                  <p
                    className={`text-xs mt-2 ${msg.sender === "user" ? "text-white/60" : "text-muted-foreground"}`}
                  >
      ...
      <p className="text-xs text-muted-foreground text-center">
        🤖 AI-assisted answers based on NCERT curriculum. Always verify with
        your textbook.
      </p>
    </div>
  );
