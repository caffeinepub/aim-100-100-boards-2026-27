...
export function getCompletionCategory(pct: number): {
  label: string;
  color: string;
  bg: string;
} {
...
export function useSyllabusProgress() {
  ...
  const getSubjectCompletion = useCallback(
    (subject: string): number => {
      ...
      if (total === 0) return 0;
      const completed = subjectData.chapters.filter(
        (ch) => progress[subject]?.[ch]?.completed,
      ).length;
  ...
  const getOverallCompletion = useCallback((): number => {
    let total = 0;
    let completed = 0;
    for (const subject of ncertSubjects) {
      total += subject.chapters.length;
      completed += subject.chapters.filter(
        (ch) => progress[subject.name]?.[ch]?.completed,
      ).length;
    }
  ...
  const isChapterCompleted = useCallback(
    (subject: string, chapter: string): boolean => {
